...
class LocationWatcher extends Component {
    ...
    canGeolocate() {
        ...
        const supported =
            provider && provider.watchPosition && provider.clearWatch;
        return supported;
    }
    ...
    makeNarrativeMarkers() {
        ...
        narratives.forEach(narrative => {
            let mappableEvents = filter(
                narrative.starts_at,
                this.eventIsMappable
            );
            markers = markers.concat(
                map(mappableEvents, event =>
                    this.makeMarker("nrt", event, () => {
    ...
    makeEventMarkers() {
        ...
        const markers = map(mappableEvents, event =>
            this.makeMarker("evt", event, () => {
                this.sendEvent("event", event.name);
    ...
    componentDidMount() {
        if (this.canGeolocate()) {
            ...
            const watchId = provider.watchPosition(
                position => this.handlePositionChange(position),
    ...
    handlePositionError(error) {
        ...
        this.setState({
            error: `Acquisition of position failed because of ${msg}`
        });
        this.stopWatchingPosition();
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                >
                    {narrativeId
                        ? this.makeEventMarkers()
                        : this.makeNarrativeMarkers()}
